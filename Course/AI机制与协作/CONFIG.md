...
`06-行动与投入/配图/06-seven-slides.html` 为自包含的 16:9 HTML 演示，不依赖外部资源。浏览器直接打开或通过既有 build/preview 访问。方向键翻页，空格播放或暂停，重播重新呈现当前页的步骤。第 1 页任务点过渡到第 2 页评分坐标，第 3–6 页保留坐标并高亮，第 7 页总结；各页动画完成后停留。任务与分镜快照存于 `06-行动与投入/案例/七页演示内容.json`，HTML 内嵌同一数据；更新时须同步两处。原评分四象限保留。
