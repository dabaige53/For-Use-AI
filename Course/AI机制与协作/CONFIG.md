...
独立网页源文件为 `06-行动与投入/配图/06-task-map.html`，内联样式、SVG 与交互，无外部依赖。使用下文既有 build、check、preview 命令，访问 `/06-行动与投入/配图/06-task-map.html`。本轮仅制作任务总览页：四栏各八项，颜色筛选、键盘点选及局部坐标提示；不替换文章正文。文字与映射以该 HTML 为此预览的真源，坐标为相对示意而非实测评分。
