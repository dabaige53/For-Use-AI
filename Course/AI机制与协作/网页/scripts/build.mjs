...
const terrainDisplay = JSON.parse(await readFile(join(course, "04-表达需求与反馈/案例/gif-display.json"), "utf8"));
...
function renderArticle(item) {
  ...
  for (const rule of ["link_open", "image"]) {
    ...
    md.renderer.rules[rule] = (tokens, idx, options, env, self) => {
      ...
      const crop = rule === "image" && item.slug === "04-表达需求与反馈" ? terrainDisplay[value?.split("/").pop()] : null;
      if (crop) {
